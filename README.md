<table width="100%">
  <tr>
    <td>
      <h1>👋 <span style="color:#8C00FF;">Hello!</span> I'm <span style="color:#FF0066;">Christian Andrés Viuche</span></h1>
      <h3>💻 <span style="color:#FC4F00;">Systems Engineering Student</span></h3>
      <p>
  <img src="https://img.shields.io/badge/⚙️%20%20DevOps%20%20-%238C00FF.svg?style=for-the-badge&logoWidth=40" height="40"/>
  <img src="https://img.shields.io/badge/🧪%20%20QA%20Automation%20%20-%23FF0066.svg?style=for-the-badge&logoWidth=40" height="40"/>
  <img src="https://img.shields.io/badge/☁️%20%20Cloud%20Computing%20%20-%23FC4F00.svg?style=for-the-badge&logoWidth=40" height="40"/>
</p>
    </td>
    <td width="350" align="center">
      <img src="https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExOWpqY2U5enFpMjMwZHNlbmo4eWsybHdzNXZ2bHMzemQ1eHd3bzd1YSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/hENDkVRxKsctCpuAun/giphy.gif"
            width="350" 
            alt="DevOps animation"/>
    </td>
  </tr>
</table>

---

### 🚀 <span style="color:#8C00FF;">About Me</span>

I am a student passionate about the <span style="color:#8C00FF;">**DevOps culture**</span>, <span style="color:#FF0066;">**Testing (QA)**</span>, and <span style="color:#FC4F00;">**cloud computing (AWS)**</span>.  
I am currently developing a portfolio focused on <span style="color:#FF0066;">**automation**</span>, <span style="color:#8C00FF;">**software quality**</span>, and <span style="color:#FC4F00;">**continuous delivery (CI/CD)**</span>.  

* 💡 My goal is to integrate <span style="color:#FF0066;">automation</span>, <span style="color:#8C00FF;">quality</span>, and <span style="color:#FC4F00;">best practices</span> into every stage of development.  
* 📦 Technologies I work with daily: <span style="color:#8C00FF;">**Git**</span>, <span style="color:#FF0066;">**Docker**</span>, <span style="color:#FC4F00;">**AWS**</span>, <span style="color:#8C00FF;">**Selenium**</span>, <span style="color:#FF0066;">**Postman**</span>, and <span style="color:#FC4F00;">**Jenkins**</span>.  
* 🌱 Always learning more about <span style="color:#FF0066;">**pipelines**</span>, <span style="color:#8C00FF;">**infrastructure as code**</span>, and <span style="color:#FC4F00;">**automated testing**</span>.

---

### 🛠️ <span style="color:#8C00FF;">Tech Stack</span>

<p>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/git/git-original.svg" width="45" height="45" alt="git"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original-wordmark.svg" width="45" height="45" alt="docker"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/amazonwebservices/amazonwebservices-plain-wordmark.svg" width="45" height="45" alt="aws"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/selenium/selenium-original.svg" width="45" height="45" alt="selenium"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/postman/postman-original.svg" width="45" height="45" alt="postman"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/jenkins/jenkins-original.svg" width="45" height="45" alt="jenkins"/>
</p>

---

### 💼 <span style="color:#FF0066;">Featured Projects</span>

#### 🔧 <span style="color:#8C00FF;">CI/CD Pipeline Demo</span>
Automated pipeline with Jenkins, Docker, and GitHub Actions.  
**Stack:** <span style="color:#FC4F00;">Jenkins · Docker · GitHub Actions</span>

#### 🧩 <span style="color:#FF0066;">Selenium Test Suite</span>
Automation of functional tests for a web application.  
**Stack:** <span style="color:#8C00FF;">Selenium · Java · TestNG</span>

#### ☁️ <span style="color:#FC4F00;">AWS Deployment Lab</span>
Deployment of infrastructure on AWS with Terraform.  
**Stack:** <span style="color:#FF0066;">AWS · Terraform · EC2</span>

---

### ⚡ <span style="color:#8C00FF;">Hobbies</span>

* 😺 <span style="color:#FF0066;">Cat lover</span>  
* 🎌 <span style="color:#FC4F00;">Anime fan</span>  
* 🤘 <span style="color:#8C00FF;">Passionate about rock and the electric guitar</span>
